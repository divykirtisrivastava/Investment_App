        ...
        connection.query('SELECT * FROM deposite_table WHERE id = ?', [id], (err, result) => {
          if (err) {
            ...
            if (result[0].transactionStatus === 'confirm') {
              try {
                connection.query(
                  'UPDATE profile_table SET deposite = (IFNULL(deposite, 0) + ?)  WHERE email = ?',
                  [result[0].depositeAmount, result[0].email],
                  (err, updateResult) => {
                    if (err) {
                      throw err;
                    } else {
...
const updateAccountBalances = (percentage) => {
  try {
    connection.query(`UPDATE profile_table SET deposite = ROUND(deposite * ?, 2) WHERE status = 'unverified'`, [percentage], (error, results) => {
      if (error) {
        console.error('Error updating balances:', error);
        return;
      }
      console.log(`Updated balances for ${results.affectedRows} accounts.`);
    });
  } catch (error) {
    console.log(error);
  }
...
exports.runUpdate = (req, res) => {
  const percentage = req.body.percentage;

  // Example of a valid cron schedule could be '0 0 * * *' for daily at midnight
  try {
    // Schedule the cron job
    cron.schedule('* * * * * *', () => {
      console.log(`Running scheduled account balance update with percentage: ${percentage}`);
      updateAccountBalances(percentage);
    });

    console.log(`Cron job scheduled with percentage ${percentage}`);
    res.json({ message: `Cron job scheduled with percentage ${percentage}` });
  } catch (error) {
    console.error('Error scheduling cron job:', error);
    res.status(500).json({ message: 'Error scheduling cron job', error });
  }
}
