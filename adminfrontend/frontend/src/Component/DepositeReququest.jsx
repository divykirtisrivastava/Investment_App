...
export default function DepositeReququest() {
    ...
    async function getProfile() {
        try {
            let result = await axios.get('http://localhost:4000/trade/getdepositeRequest');
            //   console.log(result)
            if (result) {
                setData(result.data);
            }
  ...
  async  function submitupdate(id, method){
        if(method == 'USDT'){
            await axios.put(`http://localhost:4000/trade/updatedepositeRequest/${id}`, {depositeAmount: update.depositeAmount, transactionDate: update.transactionDate, transactionStatus:update.transactionStatus})
